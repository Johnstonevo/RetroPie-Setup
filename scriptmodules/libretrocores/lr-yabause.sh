...
function configure_lr-yabause() {
  ...
  if [ -e /usr/lib/libretro/yabause_libretro.so ]
      then
        addEmulator 0 "$md_id-ppa" "saturn" "$md_instppa/yabause_libretro.so"

        addEmulator 0 "$md_id-ppa" "saturn-japan" "$md_instppa/yabause_libretro.so"
  fi
}
