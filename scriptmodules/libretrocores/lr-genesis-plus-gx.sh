...
###############
    ...
    then
                ...
                iniSet "custom_viewport_y" "91" "$core_config"
    ...
    then
                ...
                iniSet "custom_viewport_x" "300" "$core_config"
                iniSet "custom_viewport_y" "65" "$core_config"
                iniSet "aspect_ratio_index" "22" "$core_config"
    ...
    then
                ...
                iniSet  "video_shader" "$raconfigdir/shaders/crt/zfast-crt.cgp" "$core_config"
                iniSet  "video_shader_enable"  "true" "$core_config"
                iniSet "genesis_plus_gx_system_hw" "master system II" "$core_config"
                  chown $user:$user "$core_config"


    fi
    cp /home/$user/.config/RetroPie/gamegear/retroarch.cfg /home/$user/.config/RetroPie/gamegear/retroarch.cfg.bkp
    local core_config="$configdir/gamegear/retroarch.cfg"
    iniConfig " = " '"' "$md_conf_root/gamegear/retroarch.cfg"
    iniSet  "input_overlay" "$raconfigdir/overlays/handhelds/gg.cfg" "$core_config"
    ...
    iniSet "custom_viewport_x" "639" "$core_config"
    iniSet "custom_viewport_y" "253" "$core_config"
    iniSet "genesis_plus_gx_system_hw" "game gear" "$core_config"
    iniSet  "game_specific_options" "false" "$core_config"
    iniSet  "auto_overrides_enable" "false" "$core_config"
      chown $user:$user "$core_config"

cp /home/$user/.config/RetroPie/ggh/retroarch.cfg /home/$user/.config/RetroPie/ggh/retroarch.cfg.bkp
    local core_config="$configdir/ggh/retroarch.cfg"
    iniConfig " = " '"' "$md_conf_root/ggh/retroarch.cfg"
    iniSet  "input_overlay" "$raconfigdir/overlays/handhelds/gg.cfg" "$core_config"
    iniSet "input_overlay_enable" "true" "$core_config"
    iniSet "video_smooth" "false" "$core_config"
    iniSet "input_overlay_opacity" "1.0" "$core_config"
